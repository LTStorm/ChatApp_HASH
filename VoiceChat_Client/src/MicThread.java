        ...
        for (;;) {
            if (mic.available() >= SoundPacket.defaultDataLenght) { //we got enough data to send
                ...
                try {
                    ...
                    VoiceCallMessage m = null;
                    if (tot == 0) {//send empty packet
                        m = new VoiceCallMessage(new SoundPacket(null));
                    } else { //send data
                        ...
                        baos.flush();
                        baos.close();
                        m = new VoiceCallMessage( new SoundPacket(baos.toByteArray()));  //create message for server, will generate chId and timestamp from this computer's IP and this socket's port
                    }
