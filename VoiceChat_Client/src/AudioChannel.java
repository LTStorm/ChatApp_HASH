...
public class AudioChannel extends Thread {
    ...
    private ArrayList<VoiceCallMessage> queue = new ArrayList<VoiceCallMessage>(); //queue of messages to be played
    ...
    public void addToQueue(VoiceCallMessage m) { //adds a message to the play queue
        queue.add(m);
    }
        ...
        try {
            ...
            for (;;) { //this infinite cycle checks for new packets to be played in the queue, and plays them. to avoid busy wait, a sleep(10) is executed at the beginning of each iteration
                if (queue.isEmpty()) { //nothing to play, wait
                    ...
                    VoiceCallMessage in = queue.get(0);
                    queue.remove(in);
            ...
            System.out.println("Sound card or connection error");
            if (speaker != null) {
                speaker.close();
            }
            stop();
